...
const roles = useMapGetter('jabvoxResponseBot/getRoles');
...
const tabs = [
  {
    key: 'seats',
    label: () => t('JABVOX_RESPONSE_BOT.TABS.SEATS'),
    icon: 'i-lucide-bot',
  },
  {
    key: 'configs',
    label: () => t('JABVOX_RESPONSE_BOT.TABS.CONFIG'),
    icon: 'i-lucide-settings',
  },
  {
    key: 'documents',
    label: () => t('JABVOX_RESPONSE_BOT.TABS.DOCUMENTS'),
...
const newForm = ref({
  ...
  jabvox_response_bot_role_id: null,
...
const openEdit = config => {
  ...
  if (!editForms.value[config.id]) {
    editForms.value[config.id] = {
      ...
      jabvox_response_bot_role_id: config.jabvox_response_bot_role_id || null,
...
const roleOptions = computed(() =>
  (roles.value || []).map(r => ({ value: r.id, label: r.name_jabvox }))
);
...
const modelOptions = computed(() =>
  (aiModels.value || []).map(m => ({
    ...
    label:
      m.name_jabvox +
      (m.is_default_jabvox
        ? ` (${t('JABVOX_RESPONSE_BOT.CONFIG.MODEL_DEFAULT')})`
        : ''),
  }))
);

const usedInboxIds = computed(
  () => new Set((configs.value || []).map(c => c.inbox_id))
);
...
const onDestroyConfig = async config => {
  if (
    !window.confirm(
      t('JABVOX_RESPONSE_BOT.CONFIG.DELETE_CONFIRM', {
        name: inboxName(config.inbox_id),
      })
    )
  )
    return;
...
const onSync = async () => {
  try {
    ...
    useAlert(
      t('JABVOX_RESPONSE_BOT.DOCUMENTS.SYNC_SUCCESS').replace(
        '{count}',
        result?.synced ?? 0
      )
    );
...
const onDeleteDoc = async doc => {
  if (!window.confirm(t('JABVOX_RESPONSE_BOT.DOCUMENTS.DELETE_CONFIRM')))
    return;
  try {
...
onMounted(() => {
  ...
  store.dispatch('jabvoxResponseBot/fetchRoles');
...
<template>
  <div class="flex flex-col h-full w-full overflow-hidden bg-n-surface-1">
    ...
    <div class="flex-1 overflow-y-auto">
      <!-- ── Tab: Seats ── -->
      <div
        v-if="activeTab === 'seats'"
        class="max-w-3xl mx-auto px-6 sm:px-8 py-8 space-y-8"
      >
        <!-- Acentos section -->
        <div class="space-y-3">
          <p class="text-sm font-semibold text-n-slate-12">
            {{ t('JABVOX_RESPONSE_BOT.SEATS.SECTION_TITLE') }}
          </p>
          <p class="text-sm text-n-slate-10">
            {{ t('JABVOX_RESPONSE_BOT.SEATS.DESCRIPTION') }}
          </p>
          <div
            v-if="uiFlags.isFetchingSeats"
            class="text-sm text-n-slate-10 text-center py-8 animate-pulse"
          >
            ...
            >
              <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3 min-w-0">
                  <div
                    class="w-10 h-10 rounded-xl bg-woot-50 dark:bg-woot-900/20 flex items-center justify-center shrink-0"
                  >
                    <span
                      class="i-lucide-mic w-5 h-5 text-woot-600 dark:text-woot-400"
                    />
                  </div>
                  <div class="min-w-0">
                ...
                >
                  {{
                    seat.active_jabvox
                      ? t('JABVOX_RESPONSE_BOT.SEATS.ACTIVE')
                      : t('JABVOX_RESPONSE_BOT.SEATS.INACTIVE')
                  }}
                </span>
              </div>
            </div>
        ...
        <div class="space-y-3">
          <p class="text-sm font-semibold text-n-slate-12">
            {{ t('JABVOX_RESPONSE_BOT.ROLES.SECTION_TITLE') }}
          </p>
          <p class="text-sm text-n-slate-10">
            {{ t('JABVOX_RESPONSE_BOT.ROLES.DESCRIPTION') }}
          </p>
          <div
            v-if="uiFlags.isFetchingRoles"
            class="text-sm text-n-slate-10 text-center py-8 animate-pulse"
          >
            {{ t('JABVOX_RESPONSE_BOT.LOADING') }}
          </div>
          <div
            ...
            class="text-sm text-n-slate-10 text-center py-8"
          >
            {{ t('JABVOX_RESPONSE_BOT.ROLES.EMPTY') }}
          </div>
          <div v-else class="grid gap-3">
            <div
              v-for="role in roles"
            ...
            >
              <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3 min-w-0">
                  <div
                    class="w-10 h-10 rounded-xl bg-violet-50 dark:bg-violet-900/20 flex items-center justify-center shrink-0"
                  >
                    <span
                      class="i-lucide-user-cog w-5 h-5 text-violet-600 dark:text-violet-400"
                    />
                  </div>
                  <div class="min-w-0">
                    <p class="text-sm font-semibold text-n-slate-12 truncate">
      ...
      >
        ...
        <div class="flex items-center justify-between">
          <p class="text-sm text-n-slate-10">
            {{ t('JABVOX_RESPONSE_BOT.CONFIG.LIST_DESCRIPTION') }}
          </p>
          <button
        ...
        <div
          v-if="uiFlags.isFetchingConfigs"
          class="text-sm text-n-slate-10 text-center py-10 animate-pulse"
        >
          {{ t('JABVOX_RESPONSE_BOT.LOADING') }}
        </div>
        <template v-else>
          ...
          <div
            v-if="showNewForm"
            class="rounded-2xl border-2 border-dashed border-woot-300 bg-woot-25 dark:bg-woot-900/10 p-5 space-y-4"
          >
            <div class="flex items-center justify-between">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1.5">
                ...
                >
                  {{ t('JABVOX_RESPONSE_BOT.CONFIG.NAME_LABEL') }}
                </label>
                <input
                  v-model="newForm.name_jabvox"
                  type="text"
                  class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-n-slate-12 focus:outline-none focus:ring-1 focus:ring-woot-500"
                  :placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.NAME_PLACEHOLDER')
                  "
                />
              </div>
              <div class="space-y-1.5">
                <label
                  class="block text-xs font-medium text-slate-600 dark:text-slate-300"
                >
                  {{ t('JABVOX_RESPONSE_BOT.CONFIG.DESCRIPTION_LABEL') }}
                </label>
                <input
                  v-model="newForm.description_jabvox"
                  type="text"
                  class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-n-slate-12 focus:outline-none focus:ring-1 focus:ring-woot-500"
                  :placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.DESCRIPTION_PLACEHOLDER')
                  "
                />
              </div>
            </div>
            ...
            <div class="space-y-1.5">
              ...
              <ComboBox
                ...
                :search-placeholder="
                  t('JABVOX_RESPONSE_BOT.CONFIG.INBOX_PLACEHOLDER')
                "
              />
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1.5">
                ...
                <ComboBox
                  ...
                  :options="seatOptions"
                  :placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.SEAT_PLACEHOLDER')
                  "
                  :search-placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.SEAT_PLACEHOLDER')
                  "
              ...
              <div class="space-y-1.5">
                <label
                  class="block text-xs font-medium text-slate-600 dark:text-slate-300"
                >
                  {{ t('JABVOX_RESPONSE_BOT.CONFIG.ROLE_LABEL') }}
                </label>
                <ComboBox
                  v-model="newForm.jabvox_response_bot_role_id"
                  :options="roleOptions"
                  :placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.ROLE_PLACEHOLDER')
                  "
                  :search-placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.ROLE_PLACEHOLDER')
                  "
              ...
              <div class="space-y-1.5">
                ...
                <ComboBox
                  ...
                  :placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.MODEL_PLACEHOLDER')
                  "
                  :search-placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.MODEL_PLACEHOLDER')
                  "
              ...
              <div class="space-y-1.5">
                <label
                  class="block text-xs font-medium text-slate-600 dark:text-slate-300"
                >
                  {{ t('JABVOX_RESPONSE_BOT.CONFIG.AUDIO_MODEL_LABEL') }}
                </label>
                <ComboBox
                  v-model="newForm.jabvox_audio_model_id"
                  :options="modelOptions"
                  :placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.AUDIO_MODEL_PLACEHOLDER')
                  "
                  :search-placeholder="
                    t('JABVOX_RESPONSE_BOT.CONFIG.AUDIO_MODEL_PLACEHOLDER')
                  "
            ...
            <div class="flex items-center justify-between">
              <p class="text-sm font-medium text-n-slate-12">
                {{ t('JABVOX_RESPONSE_BOT.CONFIG.ENABLED_LABEL') }}
              </p>
              <button
                class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors shrink-0"
                :class="
                  newForm.enabled_jabvox
                    ? 'bg-woot-600'
              ...
              >
                <span
                  ...
                  :class="
                    newForm.enabled_jabvox ? 'translate-x-6' : 'translate-x-1'
                  "
                />
              </button>
            </div>
            ...
            <div class="space-y-2">
              <p class="text-xs font-medium text-slate-600 dark:text-slate-300">
                {{ t('JABVOX_RESPONSE_BOT.CONFIG.LABELS_TITLE') }}
              </p>
            ...
            <div class="flex justify-end gap-2">
              ...
              <Button
                :label="
                  uiFlags.isSaving
                    ? t('JABVOX_RESPONSE_BOT.LOADING')
                    : t('JABVOX_RESPONSE_BOT.CONFIG.SAVE')
                "
          ...
          <!-- Empty state -->
          <div
            v-if="!configs.length && !showNewForm"
            class="text-center py-12 text-sm text-n-slate-10"
          >
          ...
          >
            ...
            >
              <div class="flex items-center gap-3 min-w-0">
                <div
                  class="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center shrink-0"
                >
                  <span class="i-lucide-inbox w-4.5 h-4.5 text-slate-500" />
                </div>
                <div class="min-w-0">
                  <p class="text-sm font-semibold text-n-slate-12 truncate">
                    {{ config.name_jabvox || inboxName(config.inbox_id) }}
                  </p>
                  <p class="text-xs text-n-slate-10 truncate">
                    {{
                      config.name_jabvox
                        ? inboxName(config.inbox_id) + ' · '
                        : ''
                    }}{{
                      seatOptions.find(
                        s => s.value === config.jabvox_response_bot_seat_id
                      )?.label || t('JABVOX_RESPONSE_BOT.CONFIG.NO_SEAT')
                    }}<span v-if="config.jabvox_response_bot_role_id">
                      ·
                      {{
                        roleOptions.find(
                          r => r.value === config.jabvox_response_bot_role_id
                        )?.label
                      }}</span
                    >
                  </p>
              ...
              <div class="flex items-center gap-3 shrink-0">
                ...
                >
                  {{
                    config.enabled_jabvox
                      ? t('JABVOX_RESPONSE_BOT.CONFIG.ENABLED')
                      : t('JABVOX_RESPONSE_BOT.CONFIG.DISABLED')
                  }}
                </span>
                <i
            ...
            <!-- Expanded edit form -->
            <div
              v-if="expandedId === config.id && editForms[config.id]"
              class="border-t border-slate-100 dark:border-slate-700 px-5 py-5 space-y-4"
            >
              <!-- Enable toggle -->
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p class="text-xs text-n-slate-10 mt-0.5">
                    {{ t('JABVOX_RESPONSE_BOT.CONFIG.ENABLED_HINT') }}
                  </p>
                </div>
                <button
                  class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors shrink-0"
                  :class="
                    editForms[config.id].enabled_jabvox
                      ? 'bg-woot-600'
                      : 'bg-slate-200 dark:bg-slate-700'
                  "
                  @click="
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label
                    class="block text-xs font-medium text-slate-600 dark:text-slate-300"
                    >{{ t('JABVOX_RESPONSE_BOT.CONFIG.NAME_LABEL') }}</label
                  >
                  <input
                    v-model="editForms[config.id].name_jabvox"
                    type="text"
                    class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-n-slate-12 focus:outline-none focus:ring-1 focus:ring-woot-500"
                    :placeholder="
                      t('JABVOX_RESPONSE_BOT.CONFIG.NAME_PLACEHOLDER')
                ...
                <div class="space-y-1.5">
                  ...
                  >
                  <input
                    v-model="editForms[config.id].description_jabvox"
                    type="text"
                    class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-n-slate-12 focus:outline-none focus:ring-1 focus:ring-woot-500"
                    :placeholder="
                      t('JABVOX_RESPONSE_BOT.CONFIG.DESCRIPTION_PLACEHOLDER')
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label
                    class="block text-xs font-medium text-slate-600 dark:text-slate-300"
                    >{{ t('JABVOX_RESPONSE_BOT.CONFIG.SEAT_LABEL') }}</label
                  >
                  <ComboBox
                ...
                <div class="space-y-1.5">
                  <label
                    class="block text-xs font-medium text-slate-600 dark:text-slate-300"
                    >{{ t('JABVOX_RESPONSE_BOT.CONFIG.ROLE_LABEL') }}</label
                  >
                  <ComboBox
                    v-model="editForms[config.id].jabvox_response_bot_role_id"
                    :options="roleOptions"
                    :placeholder="
                      t('JABVOX_RESPONSE_BOT.CONFIG.ROLE_PLACEHOLDER')
                    "
                ...
                <div class="space-y-1.5">
                  <label
                    class="block text-xs font-medium text-slate-600 dark:text-slate-300"
                    >{{ t('JABVOX_RESPONSE_BOT.CONFIG.MODEL_LABEL') }}</label
                  >
                  <ComboBox
                ...
                <div class="flex flex-wrap gap-2">
                  ...
                  >
                    <input
                      type="checkbox"
                      :checked="
                        editForms[config.id].active_labels_jabvox.includes(cat)
                      "
                      class="w-3.5 h-3.5 rounded text-woot-500"
          ...
          <div v-if="configs.length > 0" class="flex items-center gap-3 pt-2">
            ...
            <p class="text-xs text-n-slate-10">
              {{ t('JABVOX_RESPONSE_BOT.CONFIG.SETUP_LABELS_HINT') }}
            </p>
          </div>
        </template>
      </div>
      ...
      <div
        ...
        class="max-w-3xl mx-auto px-6 sm:px-8 py-8 space-y-5"
      >
        <div class="flex items-start justify-between gap-4">
          <p class="text-sm text-n-slate-10 max-w-lg">
            {{ t('JABVOX_RESPONSE_BOT.DOCUMENTS.DESCRIPTION') }}
          </p>
          <button
          ...
          >
            <i
              ...
              :class="{ 'animate-spin': uiFlags.isSyncing }"
            />
            {{
              uiFlags.isSyncing
                ? t('JABVOX_RESPONSE_BOT.DOCUMENTS.SYNCING')
                : t('JABVOX_RESPONSE_BOT.DOCUMENTS.SYNC')
            }}
          </button>
        </div>
        ...
        <div
          v-if="uiFlags.isFetchingDocuments"
          class="text-sm text-n-slate-10 text-center py-10 animate-pulse"
        >
          {{ t('JABVOX_RESPONSE_BOT.LOADING') }}
        </div>
            ...
            <thead>
              ...
              >
                <th class="px-4 py-3 text-xs font-medium">
                  {{ t('JABVOX_RESPONSE_BOT.DOCUMENTS.FILE_LABEL') }}
                </th>
                <th class="px-4 py-3 text-xs font-medium">
                  {{ t('JABVOX_RESPONSE_BOT.DOCUMENTS.CATEGORY_LABEL') }}
                </th>
                <th class="px-4 py-3 text-xs font-medium text-center">
                  {{ t('JABVOX_RESPONSE_BOT.DOCUMENTS.ENABLED_LABEL') }}
                </th>
              ...
              >
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2">
                    <i
                      class="i-lucide-file-text w-4 h-4 text-slate-400 shrink-0"
                    />
                    <span
                      class="text-sm text-n-slate-12 truncate max-w-[220px]"
                      >{{ doc.name_jabvox }}</span
                    >
                  </div>
                </td>
                <td class="px-4 py-3">
                  ...
                  >
                    <option value="">
                      {{
                        t('JABVOX_RESPONSE_BOT.DOCUMENTS.CATEGORY_UNASSIGNED')
                      }}
                    </option>
                ...
                <td class="px-4 py-3 text-center">
                  ...
                  >
                    <span
                      ...
                      :class="
                        doc.enabled_jabvox ? 'translate-x-4' : 'translate-x-0.5'
                      "
                    />
                  </button>
                </td>
